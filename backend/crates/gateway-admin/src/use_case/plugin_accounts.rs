...
use crate::{
    model::{
        ...
        provider_credentials::{
            ...
            PluginAccountListQuery, PluginAccountPage, PluginAccountSaveReason,
            PluginAccountSaveResult, PreparedCredentialImport, PreparedPluginAccountSave,
        },
    },
    ports::{
...
use super::{
    map_provider_error, map_store_error, publish_committed_deferred,
    publish_credentials_and_observe_quota, required_credential, required_plugin_credential,
    validate_prepared_import, validate_prepared_rotation_facts,
};
    ...
    async fn save(
        ...
        match command {
            ...
            PreparedPluginAccountSave::Replace {
                ...
                let provider = if reason == PluginAccountSaveReason::Explicit {
                    Some(self.provider(&facts.provider_kind)?)
                } else {
                    None
                };
                ...
                let result = match reason {
                    ...
                    | PluginAccountSaveReason::Maintenance => {
                        self.accounts
                            .commit_credential_refresh(command, context)
                            .await
                    }
                }
                .map_err(|error| map_store_error(error, "plugin account save"))?;
                ...
                if let Some(provider) = provider {
                    ...
                    // 模型发现可能位于快照编译内，后台维护还持有刷新租约；
                    // 只合并发布，由 Runtime 当前调用失效目录，避免重入准备或刷新锁。
                    publish_committed_deferred(self.snapshot.as_ref(), result.config_revision)
                        .await?;
                }
                Ok(PluginAccountSaveResult {
                    config_revision: result.config_revision,
