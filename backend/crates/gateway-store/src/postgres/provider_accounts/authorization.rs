...
async fn commit_authorization_in_transaction(
    ...
    let result = match (command.pending.target(), command.credential) {
        (
            ...
            AuthorizationCredentialCommit::Create(credentials),
        ) => {
            if credentials.is_empty()
                || credentials.len() > 200
                || credentials
                    .iter()
                    .any(|credential| &credential.provider_kind != key.provider_kind())
            {
                return Err(invalid_receipt());
            }
            ...
            admin_adapter::import_result(result)?
        }
        (
            ...
            let result =
                repository::rotate_provider_account_admin_in_transaction(transaction, prepared)
                    .await
                    .map_err(|error| admin_store_error(ENTITY, error))?;
            admin_adapter::rotation_result(result, account_id.clone())?.into()
        }
    ...
    let accounts = result
        ...
        .map(|account| StoredAccount {
            ...
            credential_revision: account.credential_revision.map(|revision| revision.get()),
        })
        .collect::<Vec<_>>();
...
async fn load_receipt<'a>(
    ...
    if accounts.is_empty() || accounts.len() > 200 {
        return Err(unavailable_receipt());
    }
    Ok(Some(CredentialBatchMutationResult {
