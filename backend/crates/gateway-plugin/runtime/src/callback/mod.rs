...
use gateway_plugin_sdk::{CallContext, ErrorCode, PluginFault, Stage};
...
impl PluginCallbacks {
    ...
    {
        self.accounts
            .save_discovered_facts(context, scope, account, facts)
            .await
    }
    ...
    pub(crate) fn prepare_execution(
        ...
        if context.stage != gateway_plugin_sdk::Stage::Attempt
            || context.account_id.is_none()
            || context.credential_revision.is_none()
    ...
    pub(crate) fn prepare_management(
        ...
        if !matches!(
            context.stage,
            gateway_plugin_sdk::Stage::Management | gateway_plugin_sdk::Stage::Maintenance
        ) {
            return Err(denied());
        }
        self.prepare_scope(context, proxy)
    ...
    pub(crate) fn prepare_command_line(
        ...
        let scope = self.prepare_scope(context, None)?;
        scope.activate();
        Ok(scope)
    }
    ...
    fn call(
        ...
        Box::pin(async move {
            ...
            if method == "host.models.list" {
                return models
                    .call(&context, &call, None, &method, params, payload)
                    .await;
            }
            if matches!(
                method.as_str(),
                ...
                let model_scope = match context.stage {
                    Stage::Execution => Some(
                        Some(scope.as_ref())
                    ...
                    | Stage::CommandLine
                    | Stage::Maintenance
                    | Stage::Authentication => None,
                    _ => return Err(denied()),
                };
                return models
                    .call(&context, &call, model_scope, &method, params, payload)
                    .await;
            }
