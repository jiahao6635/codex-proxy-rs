...
async fn management_model_stream_retains_selected_identity_until_completion_and_rechecks_revocation()
 {
    ...
    let nested_request = json!({
        ...
        "body":{"model":"plugin-model","input":"bound management"}
    });
    ...
    environment
        .install_provider(
            json!({
                "provider_id":"management-unbound",
                "management_registration":model_route,
    ...
    environment
        .install_provider(
            json!({"provider_id":"example","generation_marker":"management-target"}),
    ...
    let bound_id = &instances
        .instances
        .iter()
        .find(|instance| instance.configuration["provider_id"] == "management-source")
        .unwrap()
        .id;
    let unbound_id = &instances
        .instances
        .iter()
        .find(|instance| instance.configuration["provider_id"] == "management-unbound")
        .unwrap()
        .id;
