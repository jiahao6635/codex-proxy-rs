...
async fn slow_management_call_does_not_block_an_independent_call() {
    ...
    let fast = session.call(
        "echo",
        session.context(Stage::Execution, Duration::from_millis(250)),
        json!({}),
        vec![],
    );
        ...
        calls.push(tokio::spawn(async move {
            ...
            if index % 2 == 0 {
                session
                    .call(
                        "echo",
                        session.context(Stage::Execution, Duration::from_secs(5)),
                        json!({"index": index}),
                        vec![],
                    )
                    .await?;
    ...
    let first = {
        ...
        tokio::spawn(async move {
            session
                .call(
                    "echo",
                    session.context(Stage::Execution, Duration::from_secs(2)),
                    json!("first"),
                    vec![],
                )
                .await
...
async fn waiting_for_send_order_consumes_the_original_call_deadline() {
    ...
    let waiting = session
        .call(
            "echo",
            session.context(Stage::Execution, Duration::from_millis(30)),
            json!("expired"),
            vec![],
        )
        .await;
    ...
    assert!(
        session
            .call(
                "echo",
                session.context(Stage::Execution, Duration::from_secs(1)),
                json!("following"),
                vec![],
            )
            .await
...
async fn ignored_cancellation_closes_the_incarnation_and_settles_other_calls() {
    ...
    let first = session.call(
        "hang_uncancellable",
        session.context(Stage::Execution, Duration::from_millis(100)),
        json!({}),
        vec![],
    );
...
async fn child_crash_fails_the_call_without_waiting_for_its_deadline() {
    ...
    let call = session.call(
        "crash",
        session.context(Stage::Execution, Duration::from_secs(30)),
        json!({}),
        vec![],
    );
