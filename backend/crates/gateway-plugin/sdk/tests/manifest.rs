use gateway_plugin_sdk::{
    ...
    call::provider::Registration, valid_package_path,
};
use serde_json::json;
...
#[test]
fn author_provider_group_expands_once_with_protocols_on_executor() {
    let source = json!({
        "manifestVersion": MANIFEST_VERSION,
        "name": "echo",
        "displayName": "Echo",
        ...
        "contributes": {
            "provider": {
                "capabilities": ["models", "authentication", "quota", "maintenance"],
                "inputFormats": ["openai"],
                "outputFormats": ["openai"]
            }
        },
        "permissions": ["accounts", "network"]
    });
    let manifest = Manifest::from_author_slice(&serde_json::to_vec(&source).unwrap()).unwrap();
    let executor = &manifest.contributes[&Capability::Executor];

    assert_eq!(manifest.contributes.len(), 5);
    assert_eq!(executor.id, "9acme.echo.executor");
    assert_eq!(executor.stages, [Stage::Execution]);
    assert_eq!(executor.input_formats, ["openai"]);
    assert_eq!(executor.output_formats, ["openai"]);
    assert_eq!(
        manifest.contributes[&Capability::Models].stages,
        [Stage::Management, Stage::Maintenance]
    );
}

#[test]
fn author_provider_group_rejects_duplicate_or_non_provider_capabilities() {
    for capabilities in [
        json!(["models", "models"]),
        json!(["management"]),
        json!(["executor"]),
        ...
        assert_eq!(
            Manifest::from_author_slice(&serde_json::to_vec(&source).unwrap()),
            Err(ManifestError::Invalid)
        );
    }

    for (input_formats, output_formats) in [
        (json!([]), json!(["openai"])),
        (json!(["openai"]), json!([])),
        (json!(["openai", "openai"]), json!(["openai"])),
        (json!(["openai"]), json!(["openai", "openai"])),
    ] {
        let source = json!({
            "manifestVersion": MANIFEST_VERSION,
            "name": "echo",
...
fn contribution_ids_are_owned_unique_and_stage_bounded() {
    ...
    manifest
        .contributes
        .insert(Capability::Executor, middleware);
    assert_eq!(manifest.validate(), Err(ManifestError::Invalid));
