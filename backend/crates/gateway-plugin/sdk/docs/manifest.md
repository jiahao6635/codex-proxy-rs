...
| `contributes` | 插件提供的扩展能力；作者可省略可推导字段，并可使用 Provider 业务组简写 |
...
| 阶段 | 能力 |
| --- | --- |
| `management` + `maintenance` | `models`、`authentication`、`quota` |
| `execution` | `executor`、`billing` |
| `management` + `observation` | `account_management` |
| `maintenance` | `request_profile`、`maintenance` |
...
Provider 作者用一个业务组声明执行协议和可选功能，无需重复 `id`、`version` 与阶段：
...
该简写始终展开 `executor`，并展开所列的 Provider 子能力。可选值只有 `models`、`authentication`、
`quota`、`request_profile`、`account_management`、`billing`、`maintenance`；协议列表必须非空。
同一清单不能再显式声明展开得到的能力。简写只存在于作者入口，CLI 会把归档中的 `plugin.json` 写成完整
`contributes`，`PluginBuilder` 再用同一结果自动注册。
