...
use crate::{
    CallContext, Capability, ErrorCode, PluginFault, Stage, call::provider::ExecutionInput,
};
...
pub(super) fn decode_execution_input(
    call: PluginCall,
    stages: &'static [Stage],
) -> Result<TypedCall<ExecutionInput>, PluginFault> {
    validate_stage(&call, stages)?;
    if !empty_object(&call.params) {
        return Err(invalid_input());
    }
    let request = ExecutionInput::decode(&call.payload).map_err(|_| invalid_input())?;
    Ok(typed_call(call, request, false))
}
...
pub(super) fn encode_metadata_stream<R: Serialize>(
    reply: TypedReply<R>,
) -> Result<CallReply, PluginFault> {
    let TypedReply {
        result,
    ...
    let stream = stream.ok_or_else(invalid_input)?;
    let result = serde_json::to_value(result).map_err(|_| invalid_input())?;
    Ok(CallReply::stream(result, Vec::new(), stream))
}

pub(super) fn encode_execution_stream(reply: TypedReply<Empty>) -> Result<CallReply, PluginFault> {
    let TypedReply {
        result: _,
        payload,
        stream,
