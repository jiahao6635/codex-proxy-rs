...
use crate::{
    ...
    call::{
        ...
        provider::{ProviderDescriptor, Registration},
    },
};
...
pub enum AuthorError {
    ...
    #[error("plugin provider descriptor does not match the declared capabilities")]
    Provider,
}
...
impl PluginBuilder {
    ...
    /// 配置一个 Provider 业务组；注册由 SDK 生成，不另写注册分支。
    #[must_use]
    pub fn provider(mut self, descriptor: ProviderDescriptor) -> Self {
        self.provider = Some(descriptor);
        self
    }
    ...
    /// # Errors
    ///
    /// 声明缺少处理器，或 Provider 的功能描述与能力不一致时失败。
    pub fn build(self) -> Result<ComposedPlugin, AuthorError> {
        validation::validate(&self.manifest, self.provider.as_ref(), &self.handlers)?;
        Ok(ComposedPlugin {
