...
use crate::{
    Capability, ContributionDeclaration, Contributions, PROTOCOL_VERSION, Permission, Stage,
};
...
impl Manifest {
    ...
    pub fn from_author_slice(bytes: &[u8]) -> Result<Self, ManifestError> {
        let UniqueValue(mut source) =
            serde_json::from_slice(bytes).map_err(|_| ManifestError::Invalid)?;
        expand_author_provider(&mut source)?;
        ...
        for (capability, declaration) in &self.contributes {
            ...
            if declaration.version != 1
                ...
                || (*capability == Capability::Executor
                    && (declaration.input_formats.is_empty()
                        || declaration.output_formats.is_empty()))
            {
                return Err(ManifestError::Invalid);
            }
        }
        if !self.contributes.contains_key(&Capability::Executor)
            && self.contributes.keys().any(|capability| {
                matches!(
                    capability,
                    Capability::Models
...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
struct AuthorProviderContribution {
    #[serde(default)]
...
fn expand_author_provider(source: &mut Value) -> Result<(), ManifestError> {
    ...
    let Some(provider) = contributes.remove("provider") else {
        return Ok(());
    };
    let provider: AuthorProviderContribution =
        serde_json::from_value(provider).map_err(|_| ManifestError::Invalid)?;
    ...
    if capabilities.len() != provider.capabilities.len()
        || provider.input_formats.is_empty()
        || provider.output_formats.is_empty()
    ...
    let executor = ContributionDeclaration {
        id: String::new(),
        version: 1,
        stages: Vec::new(),
    ...
    insert_author_contribution(contributes, Capability::Executor, executor)?;
    for capability in capabilities {
        insert_author_contribution(
            contributes,
            capability,
...
fn insert_author_contribution(
    contributes: &mut serde_json::Map<String, Value>,
    capability: Capability,
    declaration: ContributionDeclaration,
) -> Result<(), ManifestError> {
    let identifier = capability.identifier().to_owned();
    if contributes.contains_key(&identifier) {
        return Err(ManifestError::Invalid);
    }
    let declaration = serde_json::to_value(declaration).map_err(|_| ManifestError::Invalid)?;
    contributes.insert(identifier, declaration);
    Ok(())
}
