...
use super::{ExecutionEvent, ExecutionInput, WirePayload};
...
pub(super) fn encode_event(mut event: ExecutionEvent) -> Result<Vec<u8>, ExecutionEncodingError> {
    ...
    let failure = raw_failure(&mut event)
        .map(std::mem::take)
        .unwrap_or_default();
    pack(*b"GPE1", &event, [wire, failure])
}

pub(super) fn decode_event(bytes: &[u8]) -> Result<ExecutionEvent, ExecutionEncodingError> {
    let (mut event, [wire, failure]) = unpack(*b"GPE1", bytes)?;
    attach(raw_wire(&mut event), wire)?;
    attach(raw_failure(&mut event), failure)?;
    Ok(event)
}

pub(super) fn encode_input(input: ExecutionInput) -> Result<Vec<u8>, ExecutionEncodingError> {
    pack(*b"GPQ1", &input.request, [input.body])
}

pub(super) fn decode_input(bytes: &[u8]) -> Result<ExecutionInput, ExecutionEncodingError> {
    let (request, [body]) = unpack(*b"GPQ1", bytes)?;
    Ok(ExecutionInput {
        request,
        body: body.to_vec(),
    })
...
fn raw_failure(event: &mut ExecutionEvent) -> Option<&mut Vec<u8>> {
    Some(&mut event.failure.as_mut()?.response.as_mut()?.body)
}
