...
fn assert_packaged_resources(icon_path: &str, icon_mime: &str, icon: &[u8]) {
    ...
    assert_eq!(
        manifest.contributes[&Capability::Executor].input_formats,
        ["openai"]
    );
    assert_eq!(
        manifest.contributes[&Capability::Quota].stages,
...
fn source_manifest() -> &'static [u8] {
    br#"{
      ...
      "contributes":{
        ...
        "management":{},
        "provider":{"capabilities":["quota"],"inputFormats":["openai"],"outputFormats":["openai"]}
      },
