...
async fn launch(
    ...
    if let Some(command) = command.as_ref().filter(|_| help) {
        // 帮助在原生 Provider 初始化前返回，避免写默认画像、发现模型或初始化管理员。
        let runtime = prepare_plugins(
            &store,
            plugin_cache,
            &version,
            ProviderRegistry::default(),
            gateway_admin::ports::provider::ProviderAdminRegistry::new([])
    ...
    let plugin_runtime = prepare_plugins(
        &store,
        plugin_cache,
        &version,
        providers,
    ...
    if let Some(command) = command {
        ...
        let core = gateway_core::prepare_command_plane(
            store.core_ports(),
            plugin_runtime.provider_registry(),
        ...
        let accounts = gateway_admin::initialize_plugin_accounts(
            plugin_runtime.admin_registry(core.snapshots()),
        ...
        // CLI 不恢复主进程的准入状态，不构造 API、不初始化管理员或启动 Worker。
        let result = commands
    ...
    let admin_providers = plugin_runtime.admin_registry(core.snapshots());
    ...
    plan.extend(or_shutdown!(
        plugin_runtime,
        plugin_runtime.worker_contributions(core.snapshots())
    ));
    or_shutdown!(
        plugin_runtime,
...
fn prepare_plugins(
    ...
    admin_providers: gateway_admin::ports::provider::ProviderAdminRegistry,
    ...
    Ok(std::sync::Arc::new(
        ...
        .with_provider_ports(store.provider_ports()),
    ))
}
