...
| 接入新的模型服务与账号 | `executor`，按需组合 `models`、`authentication`、`quota`、`account_management`、`billing`、`request_profile` | [Provider](../../../backend/crates/gateway-plugin/sdk/docs/capabilities.md#provider)及 `call::provider` / `call::auth` |
...
| 自动刷新凭据、额度、模型或画像资料 | `maintenance` 及对应 Provider 能力 | [Provider](../../../backend/crates/gateway-plugin/sdk/docs/capabilities.md#provider)，不是任意 cron 或常驻任务入口 |

只声明实现了的能力。Provider 专属能力需要注册 Provider，并具有 `executor`；普通中间件或页面不需要虚构 Provider。
...
- 作者清单用 `Manifest::from_author_slice` 规范化，不手写 `package`、固定阶段或重复注册；Provider 可用业务组简写，CLI 生成平台、协议与资源摘要，版本范围不能使用全版本通配
