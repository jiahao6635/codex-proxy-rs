...
账号降智下线默认关闭，与容量熔断并行且互不影响。触发源是成功响应：Provider 比较本次请求的上游模型
与上游实际声明的模型，只有两者都在配置的档位表内且返回档位严格更低才按滑动窗口计数，返回模型正常
的成功响应清空计数，恢复探测不参与计数。达到阈值后写入账号级 Redis 降智冷却，调度立即跳过该账号。
该冷却不会自行到期，恢复 worker 按配置间隔用档位表最高档执行真实探测，只有探测成功且返回模型不再
降级才解除；上游未声明模型视为无法确认恢复。冷却携带成因，容量与降智各自适用自己的探测判据，
自适应并发下调只针对容量证据。关闭该功能会显式释放已有降智冷却，避免账号永久下线。
